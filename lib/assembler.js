...
function assembleNodeEx(node, insns, insnsList) {
    ...
    function makePatterns(properties) {
        const patterns = [];
        for (const property of properties) {
            // TODO: support rename, default and nest
            if (property.identifier) {
                const key  = property.identifier;
                const name = property.identifier;
                patterns.push({key, name});
            }
        }
        return patterns;
    }
    ...
    function makeFunction(node) {
        ...
        for (const p of node.params) {
            ...
            case N.INITIALIZE:
                ...
                break;
            case N.REST_PARAMETER:
                params.push({name: p.identifier, rest: true});
                break;
            case N.PATTERN_INITIALIZE:
                {
                    const patterns = makePatterns(p.properties);
                    params.push({patterns});
                }
                break;
            }
        }
        const kind = node.type == N.ARROW_FUNCTION ? 'Arrow' : 'Normal';
        makeInstruction(I.PUSH, {operand: {kind, name: node.name, params, ref: funcId}});
        makeInstruction(I.MAKE_FUNCTION);
    }
    ...
    case N.INCREMENT:
        assembleNode(node.operand, insns);
        makeInstruction(I.INCREMENT, {kind: node.kind});
        break;
    case N.DECREMENT:
        assembleNode(node.operand, insns);
        makeInstruction(I.DECREMENT, {kind: node.kind});
        break;
    case N.ASSIGNMENT:
        assembleNode(node.right, insns);
    ...
    case N.THROW:
        assembleNode(node.expr, insns);
        makeInstruction(I.THROW);
        break;
    case N.TRY:
        {
            const tryInsns = [];
            assembleNode(node.block, tryInsns);
            const catchInsns = [];
            assembleNode(node.catch.block, catchInsns);
            const catchId = insnsList.length;
            insnsList.push(catchInsns);
            const variable = node.catch.param.identifier;
            makeInstruction(I.PUSH_TRY, {variable, ref: catchId, tryLength: tryInsns.length});
            embedInsns(tryInsns);
            makeInstruction(I.POP_TRY);
        }
        break;
    case N.STATEMENTS:
        {
    ...
    case N.BINDINGS:
        {
            for (const binding of node.bindings) {
                if (binding.identifier) {
                    ...
                    function makeVariableDefinitions(properties) {
                        for (const property of properties) {
                            // TODO: support nest
                            if (property.identifier) {
                                makeInstruction(I.DEFINE_VARIABLE, {operand1: property.identifier, operand2: node.declarationType});
                            }
                        }
                    }

                    makeVariableDefinitions(binding.properties);
                    assembleNode(binding, insns);
                }
            }
        }
    ...
    case N.PATTERN_INITIALIZE:
        {
            assembleNode(node.initializer, insns);
            const patterns = makePatterns(node.properties);
            makeInstruction(I.EXTRACT_PATTERN, {operand1: patterns, operand2: true});
        }
        break;
