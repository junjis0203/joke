    ...
    case N.WHILE:
        {
            ...
            const breakOffset = exprInsns.length + stmtInsns.length + 4;
            const continueOffset = 0;
            makeInstruction(I.PUSH_BREAKABLE, {breakOffset, continueOffset});

            insns.splice(insns.length, 0, ...exprInsns);
            makeInstruction(I.UNI_OP, {operator: '!'});
    ...
    case N.FOR:
        {
            ...
            const breakOffset = expr2Insns.length + stmtInsns.length + expr3Insns.length + 5;
            const continueOffset = expr2Insns.length + stmtInsns.length + 3; // to expr3
            makeInstruction(I.PUSH_BREAKABLE, {breakOffset, continueOffset});

            insns.splice(insns.length, 0, ...expr2Insns);
            makeInstruction(I.UNI_OP, {operator: '!'});
            makeInstruction(I.JUMP_IF, {offset: stmtInsns.length + expr3Insns.length + 3});

            insns.splice(insns.length, 0, ...stmtInsns);

            insns.splice(insns.length, 0, ...expr3Insns);
            // need to pop expr3 result
            makeInstruction(I.POP);

            makeInstruction(I.JUMP, {offset: -(expr2Insns.length + stmtInsns.length + expr3Insns.length + 3)});
