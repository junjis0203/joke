...
function CoverParenthesizedExpressionAndArrowParameterList(scanner) {
    if (checkCharToken(scanner.token, '(')) {
        scanner.next();
...
function PrimaryExpression(scanner) {
    ...
    if (checkCharToken(scanner.token, '(')) {
        node = CoverParenthesizedExpressionAndArrowParameterList(scanner);
        return node.expr;
    }
...
function PostfixExpression(scanner) {
    ...
    if (checkCharToken(scanner.token, '++') || checkCharToken(scanner, '--')) {
        ...
        if (node.type != Node.IDENTIFIER_REFERENCE && node.type != Node.PROPERTY_REF) {
...
function AssignmentExpression(scanner) {
    ...
    let node = ArrowFunction(scanner);
    if (node) {
        return node;
    }
...
function ForDeclaration(scanner) {
    if (checkKeywordToken(scanner.token, 'const')) {
        scanner.next();
        let binding;
        binding = BindingIdentifier(scanner);
        if (binding) {
            return binding;
        }
    }
...
function ArrowFunction(scanner) {
    ...
    if (params && checkCharToken(scanner.token, '=>')) {
        scanner.next();
