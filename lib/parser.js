...
function checkKeywordToken(token, keyword) {
    if (keyword instanceof Array) {
        return token.type == Token.KEYWORD && keyword.includes(token.keyword);
    } else {
...
function processBinaryExpression(scanner, BNF, operators, keywords) {
    let node = BNF(scanner);
    while (true) {
        if (checkCharToken(scanner.token, operators)) {
            const operator = scanner.token.char;
            scanner.next();
        ...
        } else if (checkKeywordToken(scanner.token, keywords)) {
            const operator = scanner.token.keyword;
            scanner.next();
            const right = BNF(scanner);
            node = {
                type: Node.BINARY_OPERATOR,
...
function MultiplicativeExpression(scanner) {
    return processBinaryExpression(scanner, UnaryExpression, ['*', '/', '%']);
}

function AdditiveExpression(scanner) {
    return processBinaryExpression(scanner, MultiplicativeExpression, ['+', '-']);
}

function RelationalExpression(scanner) {
    // unsupport bitwise operator and jump to additive expression
    return processBinaryExpression(scanner, AdditiveExpression, ['>', '<', '>=', '<='], ['instanceof']);
    // support in?
}

function EqualityExpression(scanner) {
    return processBinaryExpression(scanner, RelationalExpression, ['==', '!=', '===', '!==']);
}

function LogicalANDExpression(scanner) {
    // unsupport bitwise operator and jump to equality expression
    return processBinaryExpression(scanner, EqualityExpression, '&&');
}

function LogicalORExpression(scanner) {
    return processBinaryExpression(scanner, LogicalANDExpression, '||');
}
...
function ThrowStatement(scanner) {
    if (checkKeywordToken(scanner.token, 'throw')) {
        const srcInfo = scanner.tokenPosInfo();
        scanner.next();
        const expr = Expression(scanner);
        if (checkAndStepCharToken(scanner, ';')) {
...
function TryStatement(scanner) {
    function CatchParameter(scanner) {
        // unsuport BindingPattern
        return BindingIdentifier(scanner);
    }
    function Catch(scanner) {
        if (!checkKeywordToken(scanner.token, 'catch')) {
            return;
        }
        scanner.next();
        if (!checkAndStepCharToken(scanner, '(')) {
        ...
        const param = CatchParameter(scanner);
        if (!checkAndStepCharToken(scanner, ')')) {
            return;
        }
    ...
    if (checkKeywordToken(scanner.token, 'try')) {
        const srcInfo = scanner.tokenPosInfo();
        scanner.next();
        const block = Block(scanner);
        if (!block) {
        ...
        // unsupport finally
        const catchClause = Catch(scanner);
        if (catchClause) {
            return {
                type: Node.TRY,
                block,
